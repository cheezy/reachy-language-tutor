...
Target deployment: unrelated households, each with a Reachy Mini Wireless in their own home. The
number of them is deliberately not fixed — assume it can grow by orders of magnitude, and prefer
decisions that do not care either way. What matters is the shape rather than the count: every robot
is on its own, in a home nobody administers, holding exactly one household's data.
...
- Python environment: `~/dev/reachy/reachy_mini_env`, on the 1.11 line. `reachy_language_tutor/pyproject.toml` holds the authoritative requirement (`reachy-mini>=1.10.0rc5`) — read it rather than trusting this line, which is the sort that goes stale. The SDK has to match the desktop app's robot service, and an app update can silently provision an older one into its own managed venv (`docs/SETUP.md`). Don't upgrade `reachy-mini` without checking the app's service version first.
- Camera, precisely: a **standalone SDK script** in simulation has no camera — use `ReachyMini(media_backend="no_media")` there. The **daemon** is a different path and does not follow that rule: in `--mockup-sim` it opens the host Mac's webcam through GStreamer `autovideosrc` and behaves "exactly like a real robot" (the SDK's own words in `daemon/daemon.py`), which is why the desktop dashboard has a camera view at all. Reading the first half as a general claim is what made D22 start by asking whether this mode has a camera; it does. Prototype face recognition against the Mac's webcam via OpenCV until real hardware is available.
